...
class JsxStaticTests(unittest.TestCase):
    def test_panel_contains_call_system_and_safe_subtitle_deletion_guard(self):
        ...
        self.assertIn("function chooseSubtitleFont", source)
        self.assertIn("textDocument.text = String(segment.text)", source)
        self.assertIn("sourceRectAtTime", source)
        self.assertIn("moveToBeginning", source)
